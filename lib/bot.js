...
async function notifyGroup(message) {
...
bot.start(async (ctx) => {
  ...
  await notifyGroup(`🎉 Novi klovn se pridružio: ${userName}!`);
...
bot.hears("👥 Grupa", async (ctx) => {
  ...
  return ctx.reply(
    "👥 Pridruži se grupi:",
    Markup.inlineKeyboard([
      Markup.button.url("👥 Otvori grupu", groupLink),
    ])
  );
});
...
bot.hears("🎚️ Level +1", async (ctx) => {
  ...
  const newLevel = r.rows[0].level;
  const userName = await getUserName(ctx.from);
  
  await notifyGroup(`🎚️ ${userName} je sada Level ${newLevel}!`);
  
  return ctx.reply(`✅ Level: ${newLevel}`, mainKeyboard());
});
...
bot.hears("🗑️ Obriši status", async (ctx) => {
  ...
  const userName = await getUserName(ctx.from);
  await notifyGroup(`🗑️ ${userName} je obrisao status`);
...
bot.on("text", async (ctx) => {
  ...
  if (pendingLocation.has(ctx.from.id)) {
    ...
    const userName = await getUserName(ctx.from);
    await notifyGroup(`📍 ${userName} je sada u: ${loc}`);
  ...
  if (pendingStatus.has(ctx.from.id)) {
    ...
    const userName = await getUserName(ctx.from);
    await notifyGroup(`💬 ${userName}: ${status}`);
