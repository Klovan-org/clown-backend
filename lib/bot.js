...
const PRIVATE_KEYBOARD = Markup.keyboard([
  ['🎟️ Pozivnica', '👥 Link grupe'],
  ['📊 Otvori App', '🤡 Moj status'],
]).resize();
...
export async function sendStatusNotification(telegramId, editedByTelegramId) {
  ...
  try {
    ...
    // Ako je neko drugi uredio profil, dohvati ime editora
    let editorName = null;
    if (editedByTelegramId && editedByTelegramId !== telegramId) {
      ...
      if (editor) {
        editorName = editor.clown_name || editor.first_name || editor.username || "Klovn";
      }
    }
...
bot.start(async (ctx) => {
  ...
  // već je član - prikaži keyboard
  if (await isMember(ctx.from.id)) {
    return ctx.reply("🤡 Već si unutra.", PRIVATE_KEYBOARD);
  }
  ...
  return ctx.reply("✅ Dobrodošao klovne 🤡", PRIVATE_KEYBOARD);
});

/* ======================
   KEYBOARD BUTTON HANDLERS (PRIVATE)
====================== */

bot.hears('🎟️ Pozivnica', async (ctx) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO invites (code, max_uses, uses, active) VALUES ($1, 1, 0, true)`,
      [code]
    );
    const botInfo = await bot.telegram.getMe();
    const link = `https://t.me/${botInfo.username}?start=${code}`;
    return ctx.reply(`🎟️ Tvoja pozivnica (1 korišćenje):\n\n${link}\n\nPošalji ovaj link osobi koju želiš da pozoveš.`);
  } catch (err) {
...
bot.hears('👥 Link grupe', async (ctx) => {
  if (!isPrivateChat(ctx)) return;
  if (!(await isMember(ctx.from.id))) return ctx.reply("⛔ Nisi član.");
...
bot.hears('📊 Otvori App', async (ctx) => {
  if (!isPrivateChat(ctx)) return;
